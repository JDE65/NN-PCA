...
RegulJ = (lambda / (2 * m)) * (sum(sum(Theta1(:, 2:end).^2, 2)) + sum(sum(Theta2(:, 2:end).^2, 2)));

J = sum(sum((-Ye).*log(H) - (1-Ye).*log(1-H), 2)) / m + RegulJ;

% grad = (X' * (sigmoid(X * theta) - y)) / m + Mod_theta;

delt3 = A3 - Ye;
delt2 = (delt3 * Theta2 .* sigmoidGradient([ones(size(Z2, 1), 1) Z2]))(:, 2:end);
